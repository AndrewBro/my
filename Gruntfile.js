...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    meta: {
      modules: 'angular.module("ui.mylib", [<%= srcModules %>]);',
      tplmodules: 'angular.module("ui.mylib.tpls", [<%= tplModules %>]);',
      all: 'angular.module("ui.mylib", ["ui.mylib.tpls", <%= srcModules %>]);',
      banner: ['/*',
               ' * <%= pkg.name %>',
               ' * <%= pkg.homepage %>\n',
  ...
  //Common ui.mylib module containing all modules for src and templates
  ...
  function findModule(name) {
    ...
    var module = {
      name: name,
      libraryPrefix: 'ui.mylib',
      moduleName: enquote('ui.mylib.' + name),
  ...
  function dependenciesForModule(name) {
    ...
    .forEach(function(contents) {
      ...
      dependencies.split(',').forEach(function(dep) {
        if (dep.indexOf('ui.mylib.') > -1) {
          var depName = dep.trim().replace('ui.mylib.','').replace(/['"]/g,'');
